...
def fast_rcnn_inference(
    ...
    Args:
        ...
        gt_classes (list[Tensor]): The ground-truth class of each proposal
        ...
        topk_per_image (int): The number of top scoring detections to return. Set < 0 to return
            all detections.
        relation_on (bool): whether to add boxes per class
    ...
    result_per_image = [
        fast_rcnn_inference_single_image(
            boxes_per_image, scores_per_image, gt_classes_per_image, image_shape, score_thresh, nms_thresh, topk_per_image, add_boxes_per_cls
        )
        for scores_per_image, boxes_per_image, gt_classes_per_image, image_shape in zip(scores, boxes, gt_classes, image_shapes)
    ]
...
def _log_classification_stats(pred_logits, gt_classes, prefix="fast_rcnn"):
    ...
    num_fg = fg_inds.nonzero(as_tuple=False).numel()  # num_fg = fg_inds.nonzero().numel()
...
def fast_rcnn_inference_single_image(
    ...
    if gt_classes is not None:
        result.gt_classes = gt_classes[filter_inds[:, 0]]

    # if add_boxes_per_cls:
    #     result.pred_boxes_per_cls = boxes_per_cls[filter_inds[:, 0]]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def inference(self, predictions: Tuple[torch.Tensor, torch.Tensor], proposals: List[Instances]):
        ...
        if proposals[0].has("gt_classes"):
            ...
            gt_classes =  [None] * len(proposals)
        image_shapes = [x.image_size for x in proposals]
        add_boxes_per_cls = self.relation_on and not self.training
