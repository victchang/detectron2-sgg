...
class RelationSampling(object):
    ...
    def __init__(
        ...
        self.fg_iou_thres = fg_iou_thres
        self.require_overlap = require_overlap
        self.num_samples_per_gt_rel = num_samples_per_gt_rel
    ...
    def from_config(cls, cfg):
        return {
            ...
            "fg_iou_thres": cfg.MODEL.ROI_RELATION_HEAD.FG_IOU_THRESHOLD,
            "require_overlap": cfg.MODEL.ROI_RELATION_HEAD.REQUIRE_BOX_OVERLAP,
            "num_samples_per_gt_rel": cfg.MODEL.ROI_RELATION_HEAD.NUM_SAMPLES_PER_GT_REL,
            "batch_size_per_image": cfg.MODEL.ROI_RELATION_HEAD.BATCH_SIZE_PER_IMAGE,
            "positive_fraction": cfg.MODEL.ROI_RELATION_HEAD.POSITIVE_FRACTION,
            "max_proposal_pairs": cfg.MODEL.ROI_RELATION_HEAD.MAX_PROPOSAL_PAIRS,
            "use_gt_box": cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX,
            "test_overlap": cfg.TEST.ROI_RELATION_HEAD.REQUIRE_OVERLAP,
        }
    ...
    def detect_relsample(self, proposals, targets):
        ...
        for i, (proposal, target) in enumerate(zip(proposals, targets)):
            ...
            pred_classes = proposal.gt_classes
            gt_boxes = target.get_rel_fields("gt_boxes")
            gt_classes = target.get_rel_fields("gt_classes").long()
            gt_relations = target.get_rel_fields("gt_relations")

            # IoU matching for object detection results
            ious = pairwise_iou(gt_boxes, pred_boxes)  # [tgt, prp]
            is_match = (gt_classes[:, None] == pred_classes[None]) & (ious > self.fg_iou_thres)  # [tgt, prp]

            # Proposal self IoU to filter non-overlap
            prp_self_iou = pairwise_iou(pred_boxes, pred_boxes)  # [prp, prp]
            if self.require_overlap and (not self.use_gt_box):
                rel_possibility = (prp_self_iou > 0) & (prp_self_iou < 1)  # not self & intersect
            else:
                num_prp = len(pred_boxes)
            ...
            rel_possibility[pred_classes == self.num_classes] = 0
            rel_possibility[:, pred_classes == self.num_classes] = 0

            img_rel_triplets, binary_rel = self.motif_rel_fg_bg_sampling(device, gt_relations, ious, is_match, rel_possibility)
    ...
    def motif_rel_fg_bg_sampling(self, device, gt_relations, ious, is_match, rel_possibility):
        """
        ...
        ground truth
        gt_relations:   # [number_gt_relations, 3] ([sub_idx, obj_idx, rel_class])
        ...
        return:
            ...
            binary_relatedness: the box pairs with that match the ground truth
                                [num_prp, num_prp]
        """
        gt_head_idxs = gt_relations[:, 0]
        gt_tail_idxs = gt_relations[:, 1]
        gt_rel_classes = gt_relations[:, 2]

        num_tgt_rels = gt_rel_classes.shape[0]
        # generate binary prp mask
        num_prp = is_match.shape[-1]
        binary_prp_head = is_match[gt_head_idxs]  # num_tgt_rel, num_prp (matched prp head)
        binary_prp_tail = is_match[gt_tail_idxs]  # num_tgt_rel, num_prp (matched prp tail)
        binary_rel = torch.zeros((num_prp, num_prp), device=device).long()
        ...
        for i in range(num_tgt_rels):
            ...
            tgt_head_idx = int(gt_head_idxs[i])
            tgt_tail_idx = int(gt_tail_idxs[i])
            tgt_rel_lab = int(gt_rel_classes[i])
            ...
            if fg_rel_i.shape[0] > self.num_samples_per_gt_rel:
                ...
                perm = npr.choice(ious_score.shape[0], p=ious_score, size=self.num_samples_per_gt_rel, replace=False)
        ...
        bg_rel_labs = torch.full([bg_rel_inds.shape[0]], self.num_predicates, dtype=torch.int64, device=device)
        ...
        if bg_rel_triplets.shape[0] > 0:
            ...
            bg_rel_triplets = torch.full((0, 3), self.num_predicates, dtype=torch.int64, device=device)
        ...
        if fg_rel_triplets.shape[0] == 0 and bg_rel_triplets.shape[0] == 0:
            ...
            bg_rel_triplets[:, 2] = self.num_predicates

        assert all(fg_rel_triplets[:, 2] < 50) and all(bg_rel_triplets[:, 2] == 50)

        return cat((fg_rel_triplets, bg_rel_triplets), dim=0), binary_rel


def build_relation_samp_processor(cfg):
    return RelationSampling(cfg)