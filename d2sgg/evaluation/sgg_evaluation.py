...
class SceneGraphAPI:
    def __init__(self, cfg, annotation_file=None, logger=None):
        ...
        :return:
        """
        self.num_rel_category = cfg.MODEL.ROI_RELATION_HEAD.NUM_PREDICATES
        self.multiple_preds = cfg.MODEL.ROI_RELATION_HEAD.MULTIPLE_PREDS
        self.iou_thres = cfg.MODEL.ROI_RELATION_HEAD.IOU_THRESHOLD
        self.use_gt_box = cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX
        self.use_gt_object_label = cfg.MODEL.ROI_RELATION_HEAD.USE_GT_LABEL
        self.logger = logger
    ...
    def get_evaluators(self):
        # feed other info
        global_container = {}
        global_container['zeroshot_triplet'] = torch.load("d2sgg/evaluation/zeroshot_triplet.pytorch",
    ...
    def evaluate(self, predictions, task):
        ...
        assert (task in ["sgcls"]) == self.use_gt_box, f"{task} task has to set ROI_RELATION_HEAD.USE_GT_BOX to {self.use_gt_box}, " \
                                                     f"since the gt has to be changed before the evaluation."
        assert (task == "predcls") == self.use_gt_object_label, f"{task} task has to set ROI_RELATION_HEAD.USE_GT_BOX to {self.use_gt_object_label}, " \
                                                     f"since the gt has to be changed before the evaluation."
        self.mode = task
        # reset results dict
        self.results = self.get_evaluators()
        gt_annotations = [self.imgToAnns[pred["image_id"]] for pred in predictions]
        for gt_annotation, pred in tqdm(zip(gt_annotations, predictions)):
    ...
    def evaluate_relation_of_one_image(self, groundtruth, prediction):
        ...
        assert all(local_container['gt_rels'][:, 2] < 50)
...
class SGRecall(SceneGraphEvaluation):
    ...
    def calculate_recall(self, global_container, local_container):
        ...
        pred_rels = np.column_stack((pred_rel_inds, rel_scores.argmax(axis=1)))
        # (N_obj * (N_obj - 1), 1)
        pred_scores = rel_scores.max(axis=1)
        # (N_obj * (N_obj - 1), N_obj * (N_obj - 1), 1)
        # (10, 10, 1)
...
class SGNGMeanZeroShotRecall(SceneGraphEvaluation):
    ...
    def generate_print_string(self):
        ...
        for k, v in self.result_dict[self.mode + '_ng_zeroshot_mean_recall'].items():
            result_str += 'ng-zmR @ %d: %.4f; ' % (k, float(v))
