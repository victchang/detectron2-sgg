...
logger = logging.getLogger("detectron2.data.datasets")
...
class VisualGenomeParser(object):
    def __init__(self, cfg, img_dir, roidb_file, dict_file, image_file,
                 transforms=None,
                 flip_aug=False):
        """
        ...
        self.relation_on = cfg.MODEL.RELATION_ON
    ...
    def load_info(self, add_bg=False):
        ...
        if add_bg:
            # follow detectron2 annotations (FG: 0 ~ K-1, BG: K)
            info['label_to_idx']['__background__'] = 150
            info['predicate_to_idx']['__background__'] = 50
            info['attribute_to_idx']['__background__'] = 0  # don't care
    ...
    def get_dicts(self, split, num_im=-1, num_val_im=0, filter_empty_rels=True, filter_non_overlap=False, filter_duplicates=True):
        ...
        filter_non_overlap = self.relation_on and filter_non_overlap and split in {'train', 'debug'}
        filter_duplicates = self.relation_on and filter_duplicates and split in {'train', 'debug'}
        # filter_empty_rels = True if split in {'debug', 'test', 'minitest'} else filter_empty_rels
        filter_empty_rels = self.relation_on and filter_empty_rels
        ...
        for i, filename in enumerate(filenames):
            ...
            # assert cv2.imread(filename).shape[0] == img_info[i]['height'], filename
...
def register_visual_genome(cfg):
    """
    ...
    num_images_to_load = {"train": -1,
                          ...
                          "minitrain": 1000,
                          "minitest": 1000}
    num_val_im = 5000 if cfg.DATASETS.VAL else 0
    vg_parser = VisualGenomeParser(cfg, IMG_DIR, ROI_DB_FILE, DICT_FILE, IMAGE_FILE)
    if load_freq_bias:
...
if __name__ == "__main__":
    ...
    annotations = DatasetCatalog.get("vg_train")